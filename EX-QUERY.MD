...
SELECT `students`.`name` `student_name`, `students`.`surname` `student_surname`, COUNT(`students`.`surname`) `number_of_exam_tests`, MAX(`exam_student`.`vote`) `vote_max`, MIN(`exam_student`.`vote`) `vote_min`
FROM `students`
